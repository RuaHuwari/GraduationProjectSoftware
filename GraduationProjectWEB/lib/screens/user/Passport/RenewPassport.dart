...
class ReNewPassport extends StatefulWidget {
  ReNewPassport({ required this.userId, required this.firstname, required this.lastname, required this.isEnglish});
  final String userId;
  final String firstname;
  final String lastname;
  final bool isEnglish;
  @override
  _ReNewPassportState createState() => _ReNewPassportState();
}
...
class _ReNewPassportState extends State<ReNewPassport> {
  ...
  Future<String> _saveApplicationIdToFirestore(String applicationId) async {
    ...
    try {
      ...
      if(_passportValidFor6Months)
        _agreementurl=await storeData.UploadImageToStorage(applicationId+'manager agreement', _renewagreement!);
      ...
      final QuerySnapshot querySnapshot = await _firestore
          ...
          .where('applicationID', isEqualTo: 7)
          .get();
      ...
      await _firestore.collection("IDApplications").add({
        ...
        if(_passportValidFor6Months)
          'reason':_renewReasonController,
        if(_passportValidFor6Months)
          'manageragreement':_agreementurl,
      });
  ...
  Future<void> _getmanageragreement() async {
    Uint8List img = await pickImage(ImageSource.gallery);
    setState(() {
      _renewagreement = img;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: buildabbbar.buildAbbbar(context, isEnglish?'Renew Passport':'تجديد جواز السفر', widget.userId, widget.firstname, widget.lastname,isEnglish,_toggleLanguage),
      body: Stack(
        children: [
          Container(
            height: MediaQuery
          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    isEnglish?'In order to Renew your Passport, you need to have these files ready:':'حتى تستطيع ان تباشر بتجديد جواز سفرك، يجب ان يكون لديك الملفات الاتية',
                    style: TextStyle(
                      fontSize: 30,
                      color: primary,
                    ),),
                  ...
                  Text(isEnglish?'3. An Agreement of your father or the one in charge of you, if you are younger than 18':'ثالثا: موافقة من الاب بحال كان مقدم الطلب اقل من 18 سنة',
                    style: TextStyle(
                      fontSize: 25,
                      color: primary,
                    ),),
                  Text(isEnglish?'4. Career proof if you are older than 18':'رابعا: اثبات مهنة اذا كان عمرك اكبر من 18',
                    style: TextStyle(
                      fontSize: 25,
                      color: primary,
                    ),),
                  Text(isEnglish?'5. Manager Agreement if you want to renew your passport when it still have more than 6 months till it expires':'خامسا: موافقة مدير الدائرة اذا كنت ترغب بتجديد جواز سفرك وهو صالح لاكثر من 6 اشهر',
                    style: TextStyle(
                      fontSize: 25,
                      color: primary,
                    ),),
                  Text(isEnglish?'6. When you come to get your passport, bring in the old one':'سادسا: عندما تذهب لمبنى الجوازات حتى تستلم جواز سفرك، احضر جواز السفر القديم',
                    style: TextStyle(
                      fontSize: 25,
                      color: primary,
                    ),),
                  Text(isEnglish?'7. You will need to pay 35 JOD to get the passport':'سيكون عليك دفع رسوم 35 دينار اردني عند تقديم الطلب',
                    style: TextStyle(
                      fontSize: 25,
                      color: primary,
                    ),),
                  Text("    "),
                  Text(
                    isEnglish?'After you get everything ready, You can proceed in this page and start filling the requirements.':'بعد ان يكون كل شيء جاهزا لديك يمكنك المباشرة بتقديم الطلب',
                    style: TextStyle(
                      fontSize: 25,
                      color: primary,
                    ),),
      ...
      child:ListView(
        ...
        children: [
          ...
          _birthCertificate == null
            ...
            label: Text(isEnglish?'Select Birth Certificate':'ارفق صورة شهادة الميلاد'),
          )
              :   Column(
            children: [
              Text(
          ...
          if (_selectedDate != null && _isUnderage(_selectedDate!))
            ...
            ElevatedButton.icon(
              ...
              icon: Icon(Icons.image),
              label: Text(isEnglish?'Select Picture of statment':'ارفق صورة لموافق ولي الامر'),
            ):Column(
              children: [
                Text(
                  isEnglish?'Selected File:':'الملف المرفق',
          ...
          if(_selectedDate!=null && _isUnderage(_selectedDate!)==false)
            ...
            ElevatedButton.icon(
              ...
              icon: Icon(Icons.image),
              label: Text(isEnglish?'Select Picture of career proof':'ارفق صورة اثبات المهنة'),
            ):Column(
              children: [
                Text(
                  isEnglish?'Selected File:':'الملف المرفق',
          ...
          CheckboxListTile(
            title: Text(isEnglish?"Will your passport stay valid for the next 6 months?":'هل جواز سفر سيكون صالح لل6 اشهر القادمة؟'),
          ...
          if (_passportValidFor6Months)
            TextFormField(
              controller: _renewReasonController,
              decoration: InputDecoration(labelText: isEnglish?"Reason for renewing":'سبب التجديد'),
              style: TextStyle(color: primary),
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return 'Please provide a reason for renewing';
                }
                return null;
              },
            ),
          if(_passportValidFor6Months)
            _renewagreement==null?
            ElevatedButton.icon(
              onPressed: _getmanageragreement,
              icon: Icon(Icons.image),
              label: Text(isEnglish?'Select Picture of Manager Agreement':'ارفق صورة لموافقة مدير الدائرة'),
            ):Column(
              children: [
                Text(
                  isEnglish?'Selected File:':'الملف المرفق',
                ...
                Image.memory(_renewagreement!,height: 100,),
              ],
            ),
          SizedBox(height: 20),
          ElevatedButton(
