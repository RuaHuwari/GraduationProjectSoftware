...
class NewID extends StatefulWidget {
  NewID({ required this.userId, required this.firstname, required this.lastname, required this.isEnglish});
final String userId;
...
@override
  ...
  Future<String> _saveApplicationIdToFirestore(String applicationId) async {
    ...
    try {
      ...
      final QuerySnapshot querySnapshot = await _firestore
          ...
          .where('applicationID', isEqualTo: 1)
          .get();
        ...
        Column(
          children: [
            TextFormField(
              ...
              decoration: InputDecoration(labelText: isEnglish?'$title ${i + 1} ID':'رفم هوية $title ${i+1}'),
            ...
            TextFormField(
              ...
              decoration: InputDecoration(labelText: isEnglish?'$title ${i + 1} Name':'اسم $title ${i+1}'),
              style: TextStyle(color: primary),
              validator: (value) {
                if (value == null || value.isEmpty) {
            ...
            ListTile(
              title: Text(
                dateControllers[i] == null
                    ...
                    : isEnglish?'Date of Birth: ${dateControllers[i].toLocal().year} - ${dateControllers[i].toLocal().month} - ${dateControllers[i].toLocal().day}':'تاريخ الميلاد:${dateControllers[i].toLocal().year} - ${dateControllers[i].toLocal().month} - ${dateControllers[i].toLocal().day}',
                style: TextStyle(color: primary),
              ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: buildabbbar.buildAbbbar(context, isEnglish?'NEW ID':'الحصول على هوية جديدة', widget.userId, widget.firstname, widget.lastname,isEnglish,_toggleLanguage),
      body: Stack(
        children: [
          Container(
            height: MediaQuery
          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    isEnglish?'In order to get New ID, you need to have these files ready:':
                    'حتى تحصل على هوية جديدة، يجب ان يكون بحوزتك الآتي',
                    style: TextStyle(
                      fontSize: 30,
                      color: primary,
                    ),),
                  ...
                  Text(isEnglish?'1. Personal image, with Blue background.':
                      'أولاً: صورة شخصية بخلفية زرقاء',
                      style: TextStyle(
                        fontSize: 20,
                        color: primary,

                      ), textAlign: TextAlign.left),
                  Text(isEnglish?'2. Birth Cirtificate.':
                      'ثانياً: صورة عن شهادة الميلاد', style: TextStyle(
                    fontSize: 20,
                    color: primary,
                  ), textAlign: TextAlign.left),
                  Text(isEnglish?'3. A Copy of the ID of one of the parents.':
                  'ثالثاً: صورة هوية لأحد الوالدين',
                    style: TextStyle(
                      fontSize: 20,
                      color: primary,
                    ),),
                  Text(isEnglish?'4. You will need to pay a fee of 2.00 JOD.':
                    'رابعاً: دفع رسوم هوية دينارين أردنين',
                    style: TextStyle(
                      fontSize: 20,
                      color: primary,
                    ),),
                  Text("    "),
                  Text(
                    isEnglish?'After you get everything ready, You can proceed in this page and start filling the requirements.':
                    'بعد ان يصبح لديك كل شي جاهزاً، يمكنك ان تكمل وتقوم بتعبئة نموذج تقديم طلب هوية',
                    style: TextStyle(
                      fontSize: 20,
                      color: primary,
                    ),),
  ...
  Widget _buildStage5() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(isEnglish?'if you are married fill in the following, if not skip them':'ان كنت متزوجاً، قم بتعبئة الآتي'),
          TextFormField(
            controller: _partnerid,
            decoration: InputDecoration(labelText: isEnglish?"Partner ID (if married)":'رقم هوية الزوج/الزوجة'),
            style: TextStyle(color: primary),
          ),
          TextFormField(
            controller: _partnerfullname,
            decoration: InputDecoration(
                labelText: isEnglish?"Partner full name (if married)":'اسم الزوج/الزوجة كاملاُ'),
            style: TextStyle(color: primary),
          ),
          TextFormField(
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
                labelText: isEnglish?"Partner Previous Family Name (if married)":'اسم العائلة السابق للزوج/الزوجة'),
            style: TextStyle(color: primary),
          ),
        ],
      ),
  ...
  Widget _buildStage6() {
    return Container(
      ...
      child:ListView(
        ...
        children: [
          TextFormField(
            decoration: InputDecoration(labelText: isEnglish?'Number of Sons':'عدد الأبناء'),
            style: TextStyle(color: primary),
          ...
          ElevatedButton.icon(
            ...
            label: Text(isEnglish?'Select Image of Applicant':'ارفق صورة شخصية'),
          ):Column(
            children: [
              Text(
                isEnglish?'Selected File:':'الصورة المختارة:',
    ...
    style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
      ...
      Image.memory(_birthCertificate!,height: 100,),
          ...
          ElevatedButton(
            ...
            child: Text(isEnglish?'Submit':'رفع الطلب'),
          ),
          SizedBox(height:5),
